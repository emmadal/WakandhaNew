...
class DemoConfig extends React.Component {
  ...
  handleSave = async () => {
    ...
    try {
      // Check WP website
      await fetch(url);
      await fetch(`${url}/wp-json`);

      // Check install plugin rnlab
      await fetch(`${url}/wp-json/rnlab-app-control/v1/settings`);

      // Check Woocommerce
      await fetch(
        `${url}/wp-json/wc/v3/products?consumer_key=${consumer_key}&consumer_secret=${consumer_secret}`,
      );
      this.setState({ loading: false });

      dispatch({
        type: 'UPDATE_DEMO_CONFIG_SUCCESS',
        payload: {
          url,
          consumer_key,
          consumer_secret,
          ...
          <ScrollView>
            <Container>
              ...
              <Input
                value={url}
                label={'Domain URL'}
                onChangeText={(value) => this.onChange('url', value)}
              />
              <Input
                value={consumer_key}
                label={'Consumer Key'}
              ...
              <Input
                ...
                onChangeText={(value) =>
                  this.onChange('consumer_secret', value)
                }
              />
              <Text
                h6
