...
const ItemVendor = ({ item, theme }) => {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() =>
        NavigationService.navigate(mainStack.chat_vendor, { store: item })
      }>
      <View style={styles.viewAvatar}>
        ...
        {item.countUnread ? (
          <Dot
            style={[styles.dot, { borderColor: theme.colors.border }]}
            size={14}
            color={green}
          />
        ) : null}
      ...
      <View style={[styles.content, { borderColor: theme.colors.border }]}>
        ...
        <View style={styles.viewDescription}>
          <Text h6 colorThird numberOfLines={1} style={styles.textDescription}>
            {item.chatEnd}
          </Text>
          {item.countUnread ? (
            <Badge
