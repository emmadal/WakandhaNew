...
const FeedItem = memo((props) => {
  ...
  const onReactionPress = async (reaction) => {
    if (reaction == null) {
      ...
      if (item.myReaction) {
        ...
        setSelectedIconName('blackPanther');
        onReaction('blackPanther', item); // there were no reaction before, and there was a single tap on the inline action button
      }
      setOtherReactionsVisible(false);
      return;
    }
  ...
  const moreArray = [IMLocalized('Partager')];

  if (item.authorID === user.id) {
    moreArray.push(IMLocalized('Supprimer'));
  } else {
    moreArray.push(IMLocalized("Bloquer l'utilisateur"));
    moreArray.push(IMLocalized('Signaler'));
  }

  moreArray.push(IMLocalized('Annuler'));

  const onMoreDialogDone = (index) => {
    if (index === moreArray.indexOf(IMLocalized('Partager'))) {
      onSharePost(item);
    }

    if (
      index === moreArray.indexOf(IMLocalized('Signaler')) ||
      index === moreArray.indexOf(IMLocalized("Bloquer l'utilisateur"))
    ) {
      onUserReport(item, moreArray[index]);
    }

    if (index === moreArray.indexOf(IMLocalized('Supprimer'))) {
      onDeletePost(item);
    }
  };
      ...
      return (
        <TruncateText
          ...
          <Text
            style={{
              color:
                item.imageOrColor && item.imageOrColor.textColor
                  ? item.imageOrColor.textColor
                  : '#fff',
              fontSize: 35,
              padding: 10,
            }}>
    ...
    <TouchableOpacity
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={[styles.titleContainer]}>
          ...
          <Text style={{ color: 'rgba(255,255,255,0.4)' }}> a publié</Text>
        </View>
        <TNTouchableIcon
          onPress={onMorePress}
      ...
      {item.imageOrColor && typeof item.imageOrColor === 'object' ? (
        <>
          {item.imageOrColor && !item.imageOrColor.isImage && (
            <View
              style={{
                height: 300,
                backgroundColor: item.imageOrColor.url,
                alignItems: 'center',
          ...
          {item.imageOrColor && item.imageOrColor.isImage && (
            <ImageBackground
              ...
              source={{ uri: item.imageOrColor.url }}>
              {renderPostText(item)}
            </ImageBackground>
          )}
        </>
      ...
      <View style={styles.footerContainer}>
        <View style={{ flexDirection: 'row' }}>
          ...
          <TNTouchableIcon
            containerStyle={styles.footerIconContainer}
            iconSource={AppStyles.iconSet.microphone}
            imageStyle={[styles.footerIcon, styles.tintColor]}
            renderTitle={true}
            onPress={didPressComment}
            appStyles={AppStyles}
