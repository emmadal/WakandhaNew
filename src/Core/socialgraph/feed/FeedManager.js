...
export default class FeedManager {
  ...
  subscribeIfNeeded = () => {
    ...
    if (!state.feed.didSubscribeToMainFeed) {
      ...
      this.discoverVideoUnsubscribe = firebasePost.subscribeToDiscoverVideos(
        this.onDiscoverVideoUpdate,
      );
  ...
  unsubscribe = () => {
    ...
    if (this.discoverVideoUnsubscribe) {
      this.discoverVideoUnsubscribe();
    }
  ...
  onDiscoverVideoUpdate = (posts) => {
    this.discoverVideoPosts = posts;
    this.hydratePostsIfNeeded();
  };
  ...
  hydratePostsIfNeeded = (skipReactionReduxUpdate = false) => {
    ...
    // discover videos
    if (this.reactions && this.discoverVideoPosts) {
      this.reduxStore.dispatch(
        setDiscoverVideoFeedPosts(
          this.hydratedPostsWithReactions(
            this.discoverVideoPosts,
            this.reactions,
            this.bannedUserIDs,
          ),
        ),
