...
export const addComment = async (
  ...
  try {
    ...
    notificationManager.sendPushNotification(
      post.author,
      IMLocalized('Wakandha'),
      commentAuthor.firstName +
        ' ' +
        IMLocalized('a commenté votre publication.'),
      'social_comment',
  ...
  try {
    ...
    if (documentSnapshots.docs.length > 0) {
      ...
      const message = followEnabled
        ? user.firstName + ' ' + IMLocalized('a aimé votre publication.')
        : user.firstName + ' ' + IMLocalized('a réagit à votre publication.');
      notificationManager.sendPushNotification(
        post.author,
        IMLocalized('Wakandha'),
        message,
 ...
 */
const addReplaytoCommentAction = (addComment) => async (
  parentCommentId,
  comment,
  commentAuthor,
  post,
  ...
  return addComment(
    { parentId: parentCommentId, ...comment },
    commentAuthor,
    post,
    followEnabled,
  );
};
export const addReplayToComment = addReplaytoCommentAction(addComment);
/**
...
export const deleteComment = async (commentId) => {
  ...
  await updatePostAfterDeleteComment(comment.data());
};

/**
 * update Conter comment post After delete comment
 * @param comment
 * @return {Promise<void>}
 */
const updatePostAfterDeleteComment = async (comment) => {
  // Fetch the current comment count
  const allCommentsForThisPost = await commentsRef
    .where('postID', '==', comment.postID)
    .get();
  const commentCount = allCommentsForThisPost.docs.length - 1;
